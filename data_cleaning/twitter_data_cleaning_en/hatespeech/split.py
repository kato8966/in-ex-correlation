...
test_ids = list(gender_ids['id'])
test_data = data.iloc[test_ids]
test_data.to_csv('hate_test.tsv', '\t', index=False)
...
race_ids = pd.read_csv('hate_test_race_id.csv')
for race in ['aa', 'w']:
    ids = list(race_ids['id'].loc[race_ids['race'] == race])
    test_data = data.iloc[ids]
    test_data.to_csv(f'hate_test_{race}.tsv', '\t', index=False)
